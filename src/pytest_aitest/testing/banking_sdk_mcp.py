"""Banking MCP server using the official MCP SDK.

Supports all three transports:
- stdio (default): ``python -m pytest_aitest.testing.banking_sdk_mcp``
- streamable-http: ``python -m pytest_aitest.testing.banking_sdk_mcp --transport streamable-http --port 8080``
- sse: ``python -m pytest_aitest.testing.banking_sdk_mcp --transport sse --port 8080``

This replaces the hand-rolled JSON-RPC banking_mcp.py with a proper
SDK-based implementation that exercises the same BankingService.
"""
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="Banking MCP server (SDK)")
    parser.add_argument(
        "--transport",
