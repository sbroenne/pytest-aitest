"""Todo MCP server using the official MCP SDK with FastMCP.

Supports all three transports:
- stdio (default): ``python -m pytest_aitest.testing.todo_sdk_mcp``
- streamable-http: ``python -m pytest_aitest.testing.todo_sdk_mcp --transport streamable-http --port 8080``
- sse: ``python -m pytest_aitest.testing.todo_sdk_mcp --transport sse --port 8080``

This replaces the hand-rolled JSON-RPC todo_mcp.py with a proper
SDK-based implementation that exercises the same TodoStore.
"""
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="Todo MCP server (SDK)")
    parser.add_argument(
        "--transport",
